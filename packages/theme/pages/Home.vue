<template>
  <div id="home">
    ...
    <div>
      <h2>Manufacturers</h2>
      <ul>
        <li v-for="manufacturer in manufacturers"><a :href="`/manufacturer/${manufacturer.id}`">{{ manufacturer.name }}</a> <b v-if="selectedManufacturer != manufacturer" @click="selectedManufacturerId = manufacturer.id">[Select]</b></li>
      </ul>
      <h2>Series</h2>
      <ul>
        <li v-for="s in series">{{ selectedManufacturer.name }} {{ s.name }} <b @click="selectedSeriesId = s.id" v-if="selectedSeries != s">[Select]</b></li>
      </ul>
      <h2>Models</h2>
      <ul>
        <li v-for="m in models">{{ selectedManufacturer.name }} {{ selectedSeries.name }} {{ m.name }}</li>
      </ul>
    </div>
...
export default {
  ...
  setup() {
    ...
    const { manufacturers, loading, selectedSeries, selectedManufacturerId, selectedManufacturer, selectedSeriesId, series, models } = useVehicleSelection();
